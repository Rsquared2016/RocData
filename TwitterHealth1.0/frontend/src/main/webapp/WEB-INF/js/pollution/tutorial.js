...
TutorialOverlay.prototype.onAdd = function() {
	...
	this.phase01.style.color = '#ffb157';
	...
	p1arrow.src = "/css/images/arrow_right.png";
	...
	p2arrow.src = "/css/images/arrow_up.png";
	...
	this.phase02.style.color = '#ffb157';
	...
	this.phase03.style.color = '#ffb157';
	...
	p3arrow.src = "/css/images/arrow_left.png";
...
TutorialOverlay.prototype.draw = function() {
	// reposition map
	this.map.fitBounds(getBoundsNyc());
	
	// explicitly disable controls
	allControls.pinSlider.active = false;
	allControls.timeControls.active = false;
	allControls.pollutantButtons.active = false;
	allControls.aboutButton.active = false;
	allControls.swapButton.active = false;
	allControls.citySelect.citySelect.disabled = true;
	
	var context = this;
	setTimeout(function() {
		// disable controls temporarily
		context.map.setOptions({
			draggable: false,
			scrollwheel: false,
			disableDoubleClickZoom: true
		});
		
		$(context.prevButton.button).show();
		$(context.nextButton.button).show();
		context.doPhase01();
	}, 2000);
}
...
TutorialOverlay.prototype.doPhase02 = function() {
	...
	$(allControls.timeControls.backControl).effect('pulsate', { times: 3 }, 500);
	$(allControls.timeControls.pauseControl).effect('pulsate', { times: 3 }, 500);
	$(allControls.timeControls.playControl).effect('pulsate', { times: 3 }, 500);
	$(allControls.timeControls.forwardControl).effect('pulsate', { times: 3 }, 500);
}
...
TutorialOverlay.prototype.doPhase03 = function() {
	...
	var pollutantButton = allControls.pollutantButtons.pollutantButtons['Pollution Sites'];
	$(pollutantButton).effect('pulsate', { times: 3 }, 500);
}
...
TutorialOverlay.prototype.doPhaseEnd = function() {
	...
	// explicitly reenable controls
	allControls.pinSlider.active = true;
	allControls.timeControls.active = true;
	allControls.pollutantButtons.active = true;
	allControls.aboutButton.active = true;
	allControls.swapButton.active = true;
	allControls.citySelect.citySelect.disabled = false;
...
function TutorialButton(map) {
	...
	$(this.buttonDiv).effect( "pulsate", {times:3}, 2000 );
	
	// event handlers
	var context = this;
	$(this.buttonDiv).bind('click', function(event) {
		if(!$(context.buttonDiv).hasClass('ui-state-disabled')) {
	...
	allControls['tutorialButton'] = this;
}