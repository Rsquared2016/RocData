...
MODEL *read_model(char *modelfile)
{
  ...
  nol_ll_orig(modelfile,&max_sv,&max_words,&ll); /* scan size of model file */
  max_words+=2;
  ll+=2;
		    ...
		    long int *totwords, long int *totdoc)
{
  ...
  //printf("max=%d\n", max_words_doc);
  max_words_doc = 1024; //should always be << 140
  ll = 65536;
  max_docs = 5; //should always be 1
  if(verbosity>=1) {
    printf("done\n"); fflush(stdout);
  }
...
void nol_ll_orig(char *file, long int *nol, long int *wol, long int *ll) 
